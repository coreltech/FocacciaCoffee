-- MIGRATION 11: UNIVERSAL UUID ALIGNMENT
-- Fixes type mismatch for sales_prices, recipes, AND supplies.

-- 1. Full Cleanup
DROP VIEW IF EXISTS public.v_catalog_costs;
DROP TABLE IF EXISTS public.catalog_composition;

-- 2. Create Table with UNIVERSAL UUID types
CREATE TABLE public.catalog_composition (
    ...
    supply_id UUID REFERENCES public.supplies(id) ON DELETE SET NULL, -- Supply also uses UUID
